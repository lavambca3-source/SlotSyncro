...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_teacher'])) {
    ...
    if ($name && $email && $dept && $_POST['password']) {
        $stmt = $pdo->prepare("INSERT INTO users (name, email, password_hash, role, department) VALUES (?, ?, ?, 'teacher', ?)");
        try {
...
if ($search) {
    $stmt = $pdo->prepare("SELECT * FROM users WHERE role='teacher' AND (name LIKE ? OR email LIKE ? OR department LIKE ?) ORDER BY created_at DESC");
    $like = "%$search%";
...
<head>
    ...
    <title>Manage Teachers - SlotSyncro Admin</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sidebar-logo { width: 32px; height: 32px; background: var(--admin-primary); border-radius: 8px; display: flex; align-items: center; justify-content: center; color: white; }
        ...
        .avatar { width: 36px; height: 36px; border-radius: 50%; background: #3b82f6; color: white; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 0.875rem; flex-shrink: 0; }
        .dept-badge { background: #dbeafe; color: #1e40af; padding: 0.2rem 0.6rem; border-radius: 1rem; font-size: 0.75rem; font-weight: 600; }
...
<aside class="sidebar">
    ...
    <nav class="nav-menu">
        ...
        <a href="admin_teachers.php" class="nav-item active"><i data-lucide="users"></i> Teachers</a>
    ...
    <div class="page-body">
        <div class="page-header">
            ...
            <button class="btn btn-primary" onclick="document.getElementById('addModal').classList.add('active')">
                <i data-lucide="plus" style="width:16px;height:16px;"></i> Add Teacher
            </button>
        </div>
        ...
        <form class="search-bar" method="GET">
            ...
            <?php if ($search): ?><a href="admin_teachers.php" class="btn btn-secondary">Clear</a><?php endif; ?>
        </form>
                ...
                <tbody>
                    <?php if (count($teachers) > 0): ?>
                        ...
                        <tr>
                            ...
                            <td><span class="dept-badge"><?php echo htmlspecialchars($t['department'] ?? 'N/A'); ?></span></td>
                            <td><?php echo date('M d, Y', strtotime($t['created_at'])); ?></td>
                            <td>
                                <a href="?delete=<?php echo $t['id']; ?>" class="btn btn-danger btn-sm"
                    ...
                    <?php else: ?>
                        <tr><td colspan="6">
                            <div class="empty-state">
                                <i data-lucide="users" style="width:48px;height:48px;opacity:0.3;display:block;margin:0 auto 1rem;"></i>
